...
describe('SetupProjectEnvironment Use Case', () => {
  ...
  describe('Success Cases', () => {
    it('should setup frontend environment successfully', async () => {
      ...
      expect(mockProjectSetupService.setupFrontendEnvironment).toHaveBeenCalledWith(answers);
      expect(mockProjectSetupService.setupFrontendEnvironment).toHaveBeenCalledTimes(1);
      expect(mockProjectSetupService.setupBackendEnvironment).not.toHaveBeenCalled();
    });
    ...
    it('should setup backend environment successfully', async () => {
      ...
      expect(mockProjectSetupService.setupBackendEnvironment).toHaveBeenCalledWith(answers);
      expect(mockProjectSetupService.setupBackendEnvironment).toHaveBeenCalledTimes(1);
      expect(mockProjectSetupService.setupFrontendEnvironment).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Error Cases', () => {
    it('should handle setup environment errors', async () => {
      ...
      vi.mocked(mockProjectSetupService.setupFrontendEnvironment).mockRejectedValue(
