...
describe("ProjectSetupServiceImpl", () => {
	...
	describe("Success Cases", () => {
		it("should install dependencies for frontend project", async () => {
			...
			mockDependencyInstallerService.install = vi
				.fn()
		...
		it("should setup frontend environment", async () => {
			...
			mockFileTemplateService.copyTemplate = vi
				.fn()
				.mockResolvedValue(undefined);
	...
	describe("Error Cases", () => {
		it("should handle dependency installation errors", async () => {
			...
			await expect(service.installDependencies(answers)).rejects.toThrow(
				"Installation failed",
			);
		});
		...
		it("should handle frontend environment setup errors", async () => {
			...
			await expect(service.setupFrontendEnvironment(answers)).rejects.toThrow(
		...
		it("should handle backend environment setup errors", async () => {
			...
			await expect(service.setupBackendEnvironment(answers)).rejects.toThrow(
